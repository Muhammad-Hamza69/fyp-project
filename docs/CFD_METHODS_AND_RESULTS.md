...
| Job queue (Celery/Redis) | **Implemented.** Celery 5.6 over Redis, four queues split by resource profile (`cpu`/`cfd` solo/`ai`/`reports`), `acks_late` so a solve survives a worker dying, and prefetch 1 so a long task cannot hoard the queue. Verified end to end: the API dispatches, the broker routes to `ai`, a worker consumes and returns a real LightGBM prediction. Progress is written to `job_stages` in Postgres rather than Celery's ephemeral result store, so a four-hour job survives any number of client disconnects. **Not running on the deployed instance** — Vercel serverless is 30 s with no OpenFOAM, so it can enqueue but never execute; a worker needs real hardware. See `services/worker/RUNBOOK-queue.md` |
| DICOM ingestion on upload | **Real.** `dicom.js` parses the binary stream — preamble, `DICM` marker, File Meta group, transfer syntax, then the data set (Explicit and Implicit VR Little Endian). A file that is not DICOM is rejected with the reason instead of falling back to placeholder dimensions. Sample scans in `samples/` are genuine DICOM from `phantom.py` |
